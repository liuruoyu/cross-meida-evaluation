classdef GNN_L2reg < INN
    % Graph neural net with L2 regularization on the weights.
    % The regularization is set for each layer via a vector of
    % alpha parameters. It will not affect biases and weights are accessed
    % using the parameter .w
    % If alpha is set to 0 then the layer won't be considered during cost
    % evaluation. This prevents error for layers with no params.
    ...
    methods
        function obj = GNN_L2reg(lossFun)
            obj.lossFun = lossFun;
            obj.layers = {};
            obj.acts = {};
            obj.h = {};
        ...
        function addLayer(obj, l, a, from, to, alphas)
            obj.layers{end+1} = l;
            obj.acts{end+1} = a;
            obj.nwts = obj.nwts + l.nParams();
            obj.alphas{end+1} = alphas;
        ...
        function o = clone(obj)
            o = GNN_L2reg(obj.lossFun);
            for i=1:length(obj.layers)
                o.addLayer(obj.layers{i}.clone(), obj.acts{i}, obj.CT{i}.from, obj.CT{i}.to, obj.alphas{i});
            end
        ...
        function g = computeGradOnlyCell(obj)
            ...
            for i = 1:length(obj.layers)
                ...
                % L2 reg
                if obj.alphas{i} ~= 0
                    g{i}{1} = g{i}{1} + obj.alphas{i} * obj.layers{i}.w;
                    g{i}{2} = g{i}{2} + obj.alphas{i} * obj.layers{i}.b;
                end
            end
        ...
        function [cost, g] = grad_(obj, x, y)
            ...
            % Add the L2 loss over the weights
            for i = 1:length(obj.layers)
                if obj.alphas{i} ~= 0
                    cost = cost + obj.alphas{i} * 0.5 * sum(obj.layers{i}.getParams().^2);
                end
            end
        ...
        function [cost, g] = gradCell_(obj, x, y)
            ...
            for i = 1:length(obj.layers)
                if obj.alphas{i} ~= 0
                    cost = cost + 0.5 * sum(obj.layers{i}.getParams().^2);
                end
            end
            ...
            for i = 1:length(obj.layers)
                g{i} = obj.layers{i}.gradCell(obj.h(obj.CT{i}.from), obj.d(obj.CT{i}.to)) + obj.alphas{i} * obj.layers{i}.w;
            end
        end
    end
end
