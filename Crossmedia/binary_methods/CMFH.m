function results = CMFH(settings)
...
for i = 1:size(results,1)
    ...
    for j = 1:settings.iterN
        ...
        for k = 1:size(results,2)
            try
            [U1, U2, P1, P2, Y] = solveCMFH(ifea_tr1, tfea_tr1, lambda, mu,...
                gamma, settings.codelens(k));
            CMFHi_tr1 = sign((bsxfun(@minus, P1*ifea_tr1 ,mean(Y,2))));
            CMFHt_tr1 = sign((bsxfun(@minus, P2*tfea_tr1 ,mean(Y,2))));
            CMFHi_te1 = sign((bsxfun(@minus, P1*ifea_te1 ,mean(Y,2))));
            CMFHt_te1 = sign((bsxfun(@minus, P2*tfea_te1 ,mean(Y,2))));
            CMFHi_tr2 = sign((bsxfun(@minus, P1*ifea_tr2 ,mean(Y,2))));
            CMFHt_tr2 = sign((bsxfun(@minus, P2*tfea_tr2 ,mean(Y,2))));
            CMFHi_te2 = sign((bsxfun(@minus, P1*ifea_te2 ,mean(Y,2))));
            CMFHt_te2 = sign((bsxfun(@minus, P2*tfea_te2 ,mean(Y,2))));
            
            num_tr1 = size(ifea_tr1,2);
            num_tr2 = size(ifea_tr2,2);
            
            [MAPi2t1,~,resi2t1] = mAPEvaluate(CMFHi_te1',CMFHt_tr1',gnd_te1,gnd_tr1,num_tr1,'hamming');
            [MAPt2i1,~,rest2i1] = mAPEvaluate(CMFHt_te1',CMFHi_tr1',gnd_te1,gnd_tr1,num_tr1,'hamming');
            [MAPi2t2,~,resi2t2] = mAPEvaluate(CMFHi_te2',CMFHt_tr2',gnd_te2,gnd_tr2,num_tr2,'hamming');
            [MAPt2i2,~,rest2i2] = mAPEvaluate(CMFHt_te2',CMFHi_tr2',gnd_te2,gnd_tr2,num_tr2,'hamming');
