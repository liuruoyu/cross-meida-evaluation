function results = SCMseq(settings)
...
for i = 1:size(results,1)
    ...
    for j = 1:settings.iterN
        ...
        for k = 1:size(results,2)
            try
            [Wi, Wt] = trainSCM_seq(ifea_tr1, tfea_tr1, settings.codelens(k), gnd_tr1');
            SCMsi_tr1 = (ifea_tr1*Wi)'>=0;
            SCMst_tr1 = (tfea_tr1*Wt)'>=0;
            SCMsi_te1 = (ifea_te1*Wi)'>=0;
            SCMst_te1 = (tfea_te1*Wt)'>=0;
            SCMsi_tr2 = (ifea_tr2*Wi)'>=0;
            SCMst_tr2 = (tfea_tr2*Wt)'>=0;
            SCMsi_te2 = (ifea_te2*Wi)'>=0;
            SCMst_te2 = (tfea_te2*Wt)'>=0;
            
            num_tr1 = size(ifea_tr1,1);
            num_tr2 = size(ifea_tr2,1);
            
            [MAPi2t1,~,resi2t1] = mAPEvaluate(SCMsi_te1',SCMst_tr1',gnd_te1,gnd_tr1,num_tr1,'hamming');
            [MAPt2i1,~,rest2i1] = mAPEvaluate(SCMst_te1',SCMsi_tr1',gnd_te1,gnd_tr1,num_tr1,'hamming');
            [MAPi2t2,~,resi2t2] = mAPEvaluate(SCMsi_te2',SCMst_tr2',gnd_te2,gnd_tr2,num_tr2,'hamming');
            [MAPt2i2,~,rest2i2] = mAPEvaluate(SCMst_te2',SCMsi_tr2',gnd_te2,gnd_tr2,num_tr2,'hamming');
