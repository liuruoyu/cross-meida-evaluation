function results = Class(settings)
...
for i = 1:length(results)
    ...
    try
        for j = 1:settings.iterN
            ...
            [pli_tr1, ~, ~] = predict(zeros(size(ifea_tr1',1),1), sparse(ifea_tr1'), modeli,' -b 1');
            [plt_tr1, ~, ~] = predict(zeros(size(tfea_tr1',1),1), sparse(tfea_tr1'), modelt,' -b 1');
            [pli_te1, ~, ~] = predict(zeros(size(ifea_te1',1),1), sparse(ifea_te1'), modeli,' -b 1');
            [plt_te1, ~, ~] = predict(zeros(size(tfea_te1',1),1), sparse(tfea_te1'), modelt,' -b 1');
            [pli_tr2, ~, ~] = predict(zeros(size(ifea_tr2',1),1), sparse(ifea_tr2'), modeli,' -b 1');
            [plt_tr2, ~, ~] = predict(zeros(size(tfea_tr2',1),1), sparse(tfea_tr2'), modelt,' -b 1');
            [pli_te2, ~, ~] = predict(zeros(size(ifea_te2',1),1), sparse(ifea_te2'), modeli,' -b 1');
            [plt_te2, ~, ~] = predict(zeros(size(tfea_te2',1),1), sparse(tfea_te2'), modelt,' -b 1');
            
            num_tr1 = size(gnd_tr1,2);
            num_te1 = size(gnd_te1,2);
            num_tr2 = size(gnd_tr2,2);
            num_te2 = size(gnd_te2,2);
            
            Ci_tr1 = zeros(size(gnd_tr1));
            Ct_tr1 = zeros(size(gnd_tr1));
            Ci_te1 = zeros(size(gnd_te1));
            Ct_te1 = zeros(size(gnd_te1));
            Ci_tr2 = zeros(size(gnd_tr2));
            Ct_tr2 = zeros(size(gnd_tr2));
            Ci_te2 = zeros(size(gnd_te2));
            Ct_te2 = zeros(size(gnd_te2));
            
            for k = 1:num_tr1
                Ci_tr1(pli_tr1(k),k) = 1;
                Ct_tr1(plt_tr1(k),k) = 1;
            end
            for k = 1:num_te1
                Ci_te1(pli_te1(k),k) = 1;
                Ct_te1(plt_te1(k),k) = 1;
            end
            for k = 1:num_tr2
                Ci_tr2(pli_tr2(k),k) = 1;
                Ct_tr2(plt_tr2(k),k) = 1;
            end
            for k = 1:num_te2
                Ci_te2(pli_te2(k),k) = 1;
                Ct_te2(plt_te2(k),k) = 1;
            end
            
            [MAPi2t1,~,resi2t1] = mAPEvaluate(Ci_te1',Ct_tr1',gnd_te1,gnd_tr1,num_tr1,'cosine');
            [MAPt2i1,~,rest2i1] = mAPEvaluate(Ct_te1',Ci_tr1',gnd_te1,gnd_tr1,num_tr1,'cosine');
            [MAPi2t2,~,resi2t2] = mAPEvaluate(Ci_te2',Ct_tr2',gnd_te2,gnd_tr2,num_tr2,'cosine');
            [MAPt2i2,~,rest2i2] = mAPEvaluate(Ct_te2',Ci_tr2',gnd_te2,gnd_tr2,num_tr2,'cosine');
