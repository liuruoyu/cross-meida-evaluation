...
for i = 1:nV % loop over different views to get corresponding W and D matrices
    ...
    if (isfield(options,'Dev') && options.Dev)
        ...
        dataCell{i,1}.data(isnan(dataCell{i,1}.data)) = 0;
    end
...
switch lower(method)
    ...
    case 'pls'
        ...
        X = (Wout{1,1}.alignCol)';
        Y = (Wout{2,1}.alignCol)';
        [Wpls,evals,~,~] = PLS_basesLatest(X,Y,min(options.Factor,min(Dim)),0,0);
        Wout{1,1}.Bases = Wpls;
        Wout{2,1}.Bases = evals;
    ...
    case 'lda'
        ...
        opts.disp = 0;
        [eigVec eigVal] = eigs(Wf,Df,min(options.Factor,min(Dim)),'LM',opts);
