function llh = evaluate_obj_scalarw(SX, SY, SXY, OXY, U, V, wx, wy, wxy, hyper)
    ...
    % 3. lnP(wx)
    llh3 = 0.5*log(2*phix/pi)-0.5*phix*wx*wx;
    % 4. lnP(Wy)
    llh4 = 0.5*log(2*phiy/pi)-0.5*phiy*wy*wy;
    % 5. lnP(Sx)
    llh5 = (I*(I+1)/4)*log(thetax/(2*pi))...
        -0.5*thetax*sum(sum(triu((SX-logsig(wx*(U'*U))).^2)));
    % 6. lnP(Sy)
    llh6 = (J*(J+1)/4)*log(thetay/(2*pi))...
        -0.5*thetay*sum(sum(triu((SY-logsig(wy*(V'*V))).^2)));
    % 7. lnP(Sxy)
    llh7 = 0;
    id = find(OXY>0);
    Th = logsig(wxy*(U'*V));
